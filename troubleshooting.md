...
### 문제 상황
- 드롭다운 input에 값을 입력하고 Arrow Down + Enter를 눌렀지만 옵션이 선택되지 않음 [web:12][web:18]
- `send_keys()`로 입력한 값이 화면에는 보이지만, 폼 validation이 인식하지 못하고 "생성" 버튼이 활성화되지 않음 [web:34][web:39]

### 문제 발생 조건
- MUI Autocomplete로 `<input role="combobox">`를 사용 [web:21]
- 일반 `send_keys()`로는 React의 `onChange` 이벤트가 트리거되지 않음 [web:33][web:38][web:39]
- 드롭다운 옵션(`li[role='option']`)이 DOM에 렌더링되기 전에 키 입력하면 무시됨 [web:12][web:17]
- 옵션 선택 후 포커스가 그대로 input에 남아있어서 blur 이벤트가 발생하지 않음 [web:33]
...
- 헤더 클릭 후 "생성" 버튼이 `element_to_be_clickable` 상태가 되는지 확인 [web:27][web:30]
...
### 문제 발생 조건
- locator를 잘못 잡고 있었음
- MUI Select는 `<div role="combobox">`를 사용하며 **타이핑 불가** [web:21]
...
- React 기반 MUI 컴포넌트는  
  **겉모습이 아닌 DOM 구조(`input` vs `div`)와 `role` 속성 기준으로 판단해야 한다.**
