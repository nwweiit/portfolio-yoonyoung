...
→ 따라서 `run_all_on_local.sh`라는 보조 스크립트를 작성
  - CI 환경에서의 실행 흐름(run_jmeter.sh → analyze_result.py)을 로컬에서 동일하게 검증하기 위해 작성
  - 해당 스크립트 하나의 동작으로 `run_jmeter.sh`로 `jxm파일`을 실행, `analyze_result.py`에서 그대로 나온 `jtl파일`을 분석실행

→ 그러나 다음과 같은 문제가 반복적으로 발생
...
결과적으로 PowerShell 전용 스크립트는 **근본적인 실행 안정성 문제**를 해결하지 못했고, 동시에 유지보수 비용을 증가시키는 방향이라고 판단하여 채택하지 않음.
...
문제의 원인이 Python 설치 여부가 아니라 **환경마다 다른 Python 실행 진입점**에 있음을 인식.
...
- Jenkins CI 파이프라인(run_jmeter.sh, analyze_result.py)의 기존 동작 방식에는 변경 없음
...
- CI 기준으로 작성된 자동화 스크립트는 별도 설계 없이는 로컬 환경에서 동일한 실행 진입점을 보장하지 않음
- 크로스 플랫폼 자동화에서는 **“설치 여부”보다 “실행 방식 차이”가 더 큰 리스크 요인**이 될 수 있음.
