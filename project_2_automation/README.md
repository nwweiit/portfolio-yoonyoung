...
- API 테스트
  - 담당 도메인 테스트 코드 작성
    ...
    - PFS : 2
  - 로그인 로직 코드 및 공통 실행 환경 코드 작성 : requirements.txt / .gitignore / common_conftest.py / config.py / .env.sample / api_client.py / token_manager.py 
- 성능 테스트 파이프라인
...
> Failed 처리된 하나는 사이트의 불안정성에 대처하지 못한 flaky test로, 개발중 서버라 안정화가 되지 않아 실행할때마다 실행결과가 달라졌습니다.
