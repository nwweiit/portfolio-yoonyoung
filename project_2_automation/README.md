...
- 팀프로젝트 기간 : 2025.12.19 - 2026.01.07(총 12일)
...
- OAuth 기반 인증에서 발급된 토큰을 환경 변수(.env)로 관리하고 pytest fixture로 재사용하는 구조 설계
- 공용 클라우드 환경 제약을 고려해 Load–Spike–Recovery–Soak 성능 오케스트레이션 설계
- SLA 기반 성능 기준 정의 및 자동화된 결과 판단 구조 설계
...
- API 테스트
  - 담당 도메인 테스트 코드 작성
    ...
    - PFS : 2 tests
  - 로그인 및 인증 안정화 : api_client.py / [token_manager.py](src/common/auth/token_manager.py)
  - 실행 환경 표준화 : requirements.txt / .gitignore / common_conftest.py / [config.py](src/common/auth/config.py) / .env.sample
- 성능 테스트 파이프라인 전체 구조 설계 및 구현
  - 담당 도메인 테스트용 jmx 파일 작성
    - Network : 8
  - 실행 오케스트레이션 : [run_all_on_local.sh](src/performance/scripts/run_all_on_local.sh)
  - JMeter 실행 표준화 : run_jmeter.sh
  - 결과 분석 및 SLA 기반 판단 : [analyze_result.py](src/performance/scripts/analyze_result.py) / analyze_soak_result.py / analyze_spike_result.py
  - safety net 구축을 위한 cleanup : [cleanup_entry.py](src/performance/cleanup/cleanup_entry.py) / cleanup_function.py
- CI 파이프라인: Jenkins 기반 실행 구조 설계([Jenkinsfile](src/ci/Jenkinsfile))
...
> SLA 기준은 반복 성능 테스트를 통해 측정한 baseline을 기반으로 정의했습니다.(대표 SLA 기준: Avg < 1000ms / P95 < 2000ms / Error Rate < 1%)
