...
- API 테스트
  - 담당 도메인 테스트 코드 작성
    ...
    - PFS : 2 tests
  - 로그인 로직 코드 및 공통 실행 환경 코드 작성 : requirements.txt / .gitignore / common_conftest.py / config.py / .env.sample / api_client.py / [token_manager.py](project_2_automation/src/common/auth/token_manager.py) 
- 성능 테스트 파이프라인
