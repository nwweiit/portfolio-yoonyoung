...
### 📊 API Test Execution Result (Allure Report)

아래 이미지는 **로컬 환경에서 실행한 API 테스트의 Allure 리포트 예시**입니다.  
프로젝트에서는 Jenkins 기반 CI 파이프라인과 Allure 연동 구조를 설계했으며, 해당 리포지토리에는 재현 가능한 실행 증거로 로컬 테스트 결과를 첨부했습니다.
...
![allure report](../assets/images/project_result/project2_api_allure.png)
